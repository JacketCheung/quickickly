...
class ZeroStrategy(CtaTemplate):
    ...
    def onOrder(self, order):
        ...
        if order.status == STATUS_NOTTRADED:
            ...
            print( 'time',self.am.tick.datetime,order.orderTime,order.price,self.genOrder)
...
#----------------------------------------------------------------------
    ...
    def checkCancelOrder(self):
        if self.orderMinute:
            print('orderminute',self.buyOrder,self.shortOrder,self.sellOrder,self.coverOrder)
            if self.buyOrder :
                if  self.am.macd > 0 :
